...
class ResultsFromYoavRun:
    ...
    def __init__(self, cheerio_path, result, cube_density, simulatiokon_variables_dic):
...
if __name__ == '__main__':
  ...
  #  cube_densities = [100, 150, 200, 225, 250, 275, 300, 400]
    ...
    for cube_density, seed_list in zip(cube_densities, seed_lists):
        for seed in enumerate(seed_list):
                ...
                simulation_variables_dic = {'seed': seed[1], 'sigma': sigma, 'rolling': rolling,
                                            ...
                                            'max_steps': max_steps}
                current_runner = run.SimulationRunner(current_cfg, **simulation_variables_dic)
    ...
    filename = 'YoavSimulationResults_for_box_analysis/rolling_' + str(rolling) + '_sigma_' + \
